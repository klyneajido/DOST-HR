...
if ($result && $result->num_rows > 0) {
    ...
    if ($stmt->execute()) {
        ...
        $details = "Announcement title: " . ($announcement['title']) . "";
        ...
        if ($log_stmt->execute()) {
            ...
            if ($mysqli->query($delete_query)) {
                ...
                header('Location: ../view_announcements.php?success=Announcement archived successfully.');
                exit();
            } else {
                // Redirect to announcements page with error message
                header('Location: ../view_announcements.php?error=Failed to delete announcement.');
                exit();
            }
        } else {
            // Redirect to announcements page with error message for logging failure
            header('Location: ../view_announcements.php?error=Failed to log the action.');
            exit();
        }
    } else {
        // Redirect to announcements page with error message
        header('Location: ../view_announcements.php?error=Failed to archive announcement.');
        exit();
    }
} else {
    // Redirect to announcements page with error message
    header('Location: ../view_announcements.php?error=Announcement not found.');
    exit();
}
